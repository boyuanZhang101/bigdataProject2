...
def main(train_path, test_path):
    spark = SparkSession.builder.appName("CensusIncomeLR").getOrCreate()
    # Load data with header row (you may need to add header manually)
    ...
    print(f"Census test accuracy = {acc:.4f}")
    spark.stop()


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--train", required=True, help="hdfs path to adult.data (with header)")
    parser.add_argument("--test", required=True, help="hdfs path to adult.test (with header)")
